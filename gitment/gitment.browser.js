...
function renderHeader(_ref, instance) {
  ...
  likeButton.innerHTML = '\n    ' + _icons.heart + '\n    ' + (likedReaction ? '取消赞' : '赞') + '\n    ' + (meta.reactions && meta.reactions.heart ? ' \u2022 ' + meta.reactions.heart + '个赞' : '') + '\n  ';
  ...
  issueLink.href = `/go?url=` + meta.html_url;
  issueLink.target = '_blank';
  issueLink.innerText = '所有评论';
...
function renderEditor(_ref3, instance) {
  ...
  previewTab.onclick = function () {
    ...
    if (!content) {
      preview.innerText = '没有可以预览的内容';
      return;
    }

    preview.innerText = '加载预览...';
